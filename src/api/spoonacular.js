...
export async function searchByIngredients(ingredients) {
  ...
  const res = await fetch(`${BASE}?path=recipes/findByIngredients&ingredients=${query}&number=5`);
  if (!res.ok) throw new Error(`Spoonacular ${res.status}`);
  return res.json();
}
 
export async function getRecipeDetail(id) {
  const res = await fetch(`${BASE}?path=recipes/${id}/information&includeNutrition=true`);
  if (!res.ok) throw new Error('Rezept nicht gefunden');
  return res.json();
}
 
export async function getRandomRecipes() {
  const res = await fetch(`${BASE}?path=recipes/random&number=11`);
